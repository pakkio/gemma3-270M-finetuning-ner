#!/usr/bin/env python3
"""
Fine-tune Gemma3-1B-PT (base model) for Italian Text-to-SQL task with 4-bit quantization
The TRUE competitor test: 1B parameters vs Codes-1B, same size class
"""
...
def setup_model_and_tokenizer(model_name: str, lora_r: int = 16, lora_alpha: int = 32):
    """Setup Gemma3-1B-PT model and tokenizer with LoRA and 4-bit quantization"""
    logger.info(f"Loading Gemma3-1B-PT model and tokenizer: {model_name}")
    
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token
    
    print(f"Loading model {model_name} with 8-bit quantization for gradient stability...")
...
def train_text2sql_model(
    ...
    output_dir: str = "outputs/gemma3-1b-pt-text2sql",
    epochs: int = 3,
    batch_size: int = 2,
    ...
    """Train Gemma3-1B-PT for Italian text-to-SQL"""
    
    start_time = time.time()
    logger.info("🚀 Starting Gemma3-1B-PT Text-to-SQL Training")
...
def main():
    parser = argparse.ArgumentParser(description='Fine-tune Gemma3-1B-PT for Text-to-SQL')
    ...
    parser.add_argument("--output_dir", type=str, default="outputs/gemma3-1b-pt-text2sql",
