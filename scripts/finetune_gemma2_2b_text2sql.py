#!/usr/bin/env python3
"""
Fine-tune Gemma-2-2b for Italian Text-to-SQL task with 4-bit quantization
The REAL competitor test: 2B parameters vs Codes-1B
"""
...
def setup_model_and_tokenizer(model_name: str, lora_r: int = 16, lora_alpha: int = 32):
    """Setup Gemma-2-2b model and tokenizer with LoRA and 4-bit quantization"""
    logger.info(f"Loading Gemma-2-2b model and tokenizer: {model_name}")
    
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token
    
    print(f"Loading model {model_name} with 8-bit quantization for better gradient stability...")
    model = AutoModelForCausalLM.from_pretrained(
        model_name,
        torch_dtype=torch.bfloat16,
        load_in_8bit=True,
        device_map="auto",
        attn_implementation="eager"  # Required for Gemma2 training stability
    )
...
def train_text2sql_model(
    ...
    output_dir: str = "outputs/gemma2-2b-text2sql",
    epochs: int = 5,
    batch_size: int = 2,
    ...
    """Train Gemma-2-2b for Italian text-to-SQL"""
    
    start_time = time.time()
    logger.info("🚀 Starting Gemma-2-2b Text-to-SQL Training")
...
def main():
    parser = argparse.ArgumentParser(description='Fine-tune Gemma-2-2b for Text-to-SQL')
    ...
    parser.add_argument("--output_dir", type=str, default="outputs/gemma2-2b-text2sql",
