#!/usr/bin/env python3
"""
Fine-tune CodeS-1B for Italian Text-to-SQL task
Purpose-built SQL model vs Gemma3-270M comparison
"""
...
def create_training_example(question: str, sql: str) -> str:
    """Create a training example in CodeS format for Italian SQL"""
    return f"""### Task: Convert this Italian question to SQL query
### Question: {question}
### SQL: {sql}"""
...
def preprocess_function(examples, tokenizer, max_length=1024):
    ...
    for question, sql in zip(examples["question"], examples["sql"]):
        ...
        input_prompt = f"""### Task: Convert this Italian question to SQL query
### Question: {question}
### SQL: """
        ...
        # Combine for full sequence - CodeS doesn't need special BOS token
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune CodeS-1B for Italian Text-to-SQL")
    ...
    parser.add_argument("--output_dir", type=str, default="outputs/codes-1b-text2sql",
    ...
    logger.info("Starting CodeS-1B Italian Text-to-SQL fine-tuning")
    ...
    inference_template = f"""### CodeS-1B Italian Text-to-SQL Usage:
...
"""
    ...
    config = {
        ...
        "comparison_note": "CodeS-1B vs Gemma3-270M vs DistilBERT for Italian SQL generation"
    }
    
    with open(f"{args.output_dir}/training_config.json", "w") as f:
        json.dump(config, f, indent=2)
    
    logger.info("CodeS-1B Italian Text-to-SQL fine-tuning completed successfully!")
